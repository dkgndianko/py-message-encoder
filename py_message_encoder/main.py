...
from .encoders.string_encoders import FixedLengthEncoder, VariableLengthEncoder, LVAR, LLVAR, LLLVAR
...
def test():
    ...
    for encoder, cases in test_cases:
        print(f"Testing {encoder.__class__} ...")
        for case in cases:
            try:
                encoded = encoder.encode(case)
                print(f"[ENCODE] '{case}'    ->     '{encoded}' ({len(encoded)})")
